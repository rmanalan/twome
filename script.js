...
$(window).scroll(function() {
	if (($(window).scrollTop() > $(document).height() - $(window).height() - 400) && ! infScrollActive) {
		...
		var url = [$('#pagination a.more').attr('href'), $('input[name="authenticity_token"]').val()].join('&authenticity_token=');
		$.ajax({
			url: url,
			dataType: 'json',
			success: function(d) {
				var timeline = $("#timeline").append($(d["#timeline"]).find(".hentry"));
...
// Nested Twitter Replies modified to work inside a Chrome extension
// http://pratham.name/nested-twitter-replies.html
twitter_replies = {
	html : {},
	replies : [],
	count : 0,
		...
		if (typeof twitter_replies.html [reply.substr (reply.lastIndexOf ('/') + 1)] == 'undefined') {
			$.ajax({
				...
				success: function (responseDetails) {
					...
					twitter_replies.process_reply (responseDetails, id, reply);
				}
			});
		}
	...
	process_reply : function (html, id, reply) {
		...
		var thumbRegex = new RegExp ("src=\"(https?://a[1-9].twimg.com/profile_images/[^\"]*)\"", "gi");
